...
class Validator {
    ...
    filter(data) {
        let res = {};
        // 元类型 boolean in int string url float email date file custom
        // 限制类型 nullable required min max length range minlength maxlength 
    ...
    check(data) {
        ...
        for (let k in this.rules) {
            let v = data[k];
            let rule = this.rules[k];
            if (_.isEmpty(v) && rule.nullable || rule.if && _.isEmpty(data[rule.if])) {
                delete data[k];
                continue;
            }
            if (rule.required) {
